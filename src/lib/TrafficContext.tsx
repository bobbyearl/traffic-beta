...
export function TrafficProvider({ children }: { children: ReactNode }) {
  ...
  const setSelected = (ids: Set<string>) => navigate({ search: { ...params, selected: ids.size ? [...ids].join(',') : undefined } })
  const toggleCamera = (id: string) => { const next = new Set(selectedIds); if (next.has(id)) next.delete(id); else next.add(id); setSelected(next) }
  ...
  const setView = (v: string | undefined) => navigate({ search: { ...params, view: v } })
  const setMode = (m: string | undefined) => navigate({ search: { ...params, mode: m } })
  const setGrid = (g: string | undefined) => navigate({ search: { ...params, grid: g } })
  const setTab = (tab: string | undefined) => navigate({ search: { ...params, tab } })
  const setState = (s: string) => navigate({ search: { state: s === 'sc' ? undefined : s, view: params.view, mode: params.mode, grid: params.grid, tab: params.tab, selected: undefined, detail: undefined } })

  const [layoutKey, setLayoutKey] = useState(0)
  const triggerLayout = () => setLayoutKey((k) => k + 1)

  const value: TrafficState = {
    cameras, isLoading, stateId, stateConfig, selectedIds, selectedCameras, detailCam, activeRouteName,
    mode, view, cardSize, sidebarTab, layoutKey,
    toggleCamera, clearAll, selectRoute, setDetailCam, setView, setMode, setGrid, setTab, setState, triggerLayout,
  }
